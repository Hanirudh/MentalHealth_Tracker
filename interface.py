...
# Function to convert Folium map to image using PIL
...
def location():
    ...
    if user_location['latitude'] is not None:
        ...
        # Display the map with nearby psychiatrists
        st.write("## Map of Nearby Psychiatrists")
        map = display_psychiatrists_map(psychiatrists, user_location)
        st.image(folium_map_to_image(map), caption='Map Image', use_column_width=True)
    else:
...
def yogatracker():
     webbrowser.open_new_tab('https://yoga-trackerhani.netlify.app')
